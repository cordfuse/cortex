...
**Actor name resolution** — for each `.md` file in `manifest/framework/actors/` (built-ins) and `manifest/custom/actors/` (user actors), resolve the actor name using this priority (stop at first match, case-insensitively):
...
2. **Pass 2 — open items?** Two steps — do not skip the second:
   ...
   - **Step B — verify:** for every candidate, read its full source file. Also read in full every file in `data/records/` **dated (by its filename `YYYY-MM-DD`) within the past 7 days** — recency here means the record's own date, **not** its git-modified time. A bulk pass that re-commits many old records (e.g. `weave` adding `## Related` blocks across the whole corpus) must NOT drag every touched record into this read; that would make `hello` try to read the entire vault and hang. A later file may have resolved, superseded, or rendered moot an older open item even if the original file was never updated. Only surface an item as open if it is still unresolved after reading this context. Do not treat an unchecked box as ground truth without this check.
3. **Pass 3 — unresolved follow-ups?** Any file filed today with pending actions noted.
4. **Pass 4 — actor file validation.** For every entry in the `actors:` array in `context.md`, verify that a resolvable personality file exists (via the full **Actor name resolution** priority above: frontmatter `name:` → `## name` body heading → `metadata.alias` → `## aliases` → filename slug). If any actor has no resolvable file, surface it in the greeting as a warning before the open question:
...
1. **Actor introduction (always first line).** Name + one-line title pulled from the active personality file's frontmatter `name:` field and its `## Title` heading (the line beneath the heading). Use verbatim casing. One line. Example:
...
- **A serial record joins its chain first.** If the new record belongs to a dated series (weight log, meals, health daily, a numbered round), its **first** link is the immediate predecessor in that series — read the nearest same-type record before writing, and follow the pattern that series already uses. Thematic links come *after* the chain link, never instead of it, and a serial record caps at **3** (as under `weave`). Without this, each day's record links to whatever topic is loudest in session context and the series stops being traversable — the reader can no longer walk it backwards. Where the series has a gap, link the actual nearest existing record and say so in the reason, so the gap is legible.
- **Same rules as `weave`, applied one record at a time.** Convention-matching, the dated-series chain, the no-dead-edge existence check and the never-guess-between-ambiguous-candidates rule are properties of *linking*, not of the backfill verb — see the `weave` section below for each in full. Capture-time is where most links are actually born, so a rule enforced only during backfill is a rule that mostly does not run.
...
**Match the existing convention first.** Before writing a record's links, identify its type (weight log, medication record, symptom log, meeting note, …) and look at how 2–3 already-linked records of that *same type* are linked. Follow the pattern they establish rather than inventing one — a new record joins its series the way the series already works. Infer the convention once per type per session; you needn't re-derive it per record. Only introduce a new pattern when no same-type precedent exists. The dated-series rule below is the built-in default for serial records, but a convention already present in the corpus overrides it.
...
| `add actor <name>` | **Pre-commit validation (v4.2.1+):** before writing to `context.md`, verify a resolvable personality file exists for `<name>` (full **Actor name resolution** priority: frontmatter `name:` → `## name` body heading → `metadata.alias` → `## aliases` → filename slug — scans both `manifest/framework/actors/` and `manifest/custom/actors/` recursively). If no file is found, block the operation and surface: *"No personality file found for `<name>`. Create it first with `create actor <name>`, or import it with `import actor`."* **Abstract check (v4.5.2+):** if the file exists but has `## abstract: true`, block: *"[Name] is an abstract actor — for inheritance only. It can't join the room. Use it as a `## parents` target in another actor file. Run `list actors` for activatable options."* **Deprecation warning (v4.5.3+):** if the file has `## deprecated: true`, warn: *"[Name] is deprecated. Add anyway? (yes / no)"* Do not commit an actor that has no file. If the file exists, is not abstract, and user confirmed if deprecated: append a new entry. | New actor is NOT the active speaker by default — must say `change actor to <name>` separately. Surfaces Bootstrap acknowledgement: *"Oscar joined the room. Apex is still the active speaker."* Aliases: *bring in*, *invite*. Natural-language triggers: *"Hey Oscar, join us"*, *"Bring Oscar in"*. |
